...
export class AndroidResourcesV4MigrationService implements IAndroidResourcesV4MigrationService {
	...
	hasMigrated(appResourcesDir: string): boolean {
		return this.$fs.exists(path.join(appResourcesDir, AndroidResourcesV4MigrationService.ANDROID_DIR, "src", "main"));
	}
	...
	async migrate(appResourcesDir: string): Promise<void> {
		...
		shell.mv(originalAppResources, path.join(appResourcesDir, AndroidResourcesV4MigrationService.ANDROID_DIR_OLD));
...
$injector.register("androidResourcesV4MigrationService", AndroidResourcesV4MigrationService);
